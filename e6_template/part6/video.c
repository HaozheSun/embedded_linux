...
volatile int * CHAR_BUF_CTRL_ptr;
int pixel_buffer, char_buffer;
int back_buffer;
int resolution_x, resolution_y, char_resolution_x, char_resolution_y; // screen resolution
...
void plot_string(int x, int y, char * str) {
    ...
    int cur_y = y;
    while (str[i] != '\0') {
        cur_x = (x + i) % char_resolution_x;
        cur_y = y + (x + i) / char_resolution_x;
        if (cur_y >= char_resolution_y) {
            erase();
            cur_x = 0;
            cur_y = 0;
        }
        plot_char(cur_x, cur_y, str[i]);
        i++;
    }
}
...
void get_char_specs(volatile int * CHAR_BUF_CTRL_ptr) {
    char_resolution_x = *(CHAR_BUF_CTRL_ptr + 2) & 0xFFFF;
    char_resolution_y = *(CHAR_BUF_CTRL_ptr + 2)>>16 & 0xFFFF;
}
...
static int __init start_video(void)
{
    ...
    CHAR_BUF_CTRL_ptr = (unsigned int *) (LW_virtual + CHAR_BUF_CTRL_BASE);
    get_char_specs(CHAR_BUF_CTRL_ptr);
    char_buffer = ioremap_nocache (FPGA_CHAR_BASE, FPGA_CHAR_SPAN);
